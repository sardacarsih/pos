...
public partial class Frmlogin : XtraForm
{
    ...
    private void LoadLoginBackground()
    {
        using Stream? stream = typeof(Frmlogin).Assembly
            .GetManifestResourceStream("BackOffice.Assets.LoginKopkar.png");
        if (stream is null)
        {
    ...
    private void Login_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            LoginResult? res = auth.Lookup(username, AppIds.BackOffice);

            if (res is null)
            {
                XtraMessageBox.Show(
                    "UserID tidak terdaftar atau tidak punya akses BackOffice. \nAnda Memiliki " +
            ...
            {
                XtraMessageBox.Show(
                    "Akun Anda nonaktif atau terkunci - Hubungi Administrator",
                    "Info",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
                return;
            ...
            if (passwordCryptography.IsValidPassword(pwd, res.PasswordHash))
            {
                ...
                LoginInfo.AccessibleApps = auth.GetAccessibleApps(username);
                Hide();
                new BackOffice().Show();
            }
            else
